...
<script type="text/javascript">
        ...
        $('#table1').dataTable({
            ...
            "ajax" : {
                ...
                "data": function(data) {
                    ...
                    sendData.svcTpCd = $("#svcTpCd").val();
                    sendData.svcTpDtlCd = $("#svcTpDtlCd").val();
    ...
    function excel(){
        ...
        for (var i = 0; i < resultList.length; i++) {
            ...
            cJson.서비스API = resultList[i].svcTpCd;
            cJson.상세API = resultList[i].svcTpDtlCd;
        ...
        param.title = "외부API 접근이력 목록";
        param.tableHeader = "['접근일시', '아이디', '유형', '단지명', '동', '호', '서비스API', '상세API']";
        ...
        $.ajax({
            ...
            success: function(blob){
                ...
                link.download= today+"_외부API 접근이력 목록.xlsx";
                link.click();
            },
            error: function(e){
                console.log("에러");
            ...
            <table>
                <tbody>
                    ...
                    <tr>
                        ...
                        <th>· 서비스API</th>
                        <td>
                            <select name="svcTpCd" id="svcTpCd" class="custom-select">
                                <option value="ALL" <c:if test="${userTpCd eq 'ALL'}">selected</c:if>>전체</option>
                                <option value="KAKAO_MAP" <c:if test="${userTpCd eq 'KAKAO_MAP'}">selected</c:if>>카카오맵</option>
                            </select>
                        </td>
                        <th>· 상세API</th>
                        <td>
                            <select name="svcTpDtlCd" id="svcTpDtlCd" class="custom-select">
                                <option value="ALL" <c:if test="${userTpCd eq 'ALL'}">selected</c:if>>전체</option>
                                <option value="MEDICAL" <c:if test="${userTpCd eq 'MEDICAL'}">selected</c:if>>병원/약국</option>
                                <option value="PRESCHOOL" <c:if test="${userTpCd eq 'PRESCHOOL'}">selected</c:if>>유치원/어린이집</option>
                                <option value="SCHOOL" <c:if test="${userTpCd eq 'SCHOOL'}">selected</c:if>>학교</option>
                            </select>
                        </td>
                        <td>
            ...
            <table class="table" id="table1">
                <thead>
                    <tr>
                        ...
                        <th scope="col">동</th>
                        <th scope="col">호</th>
                        <th scope="col">서비스API</th>
                        <th scope="col">상세API</th>
                    </tr>
                </thead>
                <tbody>
                </tbody>
