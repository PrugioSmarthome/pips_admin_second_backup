        ...
        return this.on( 'mousedown touchstart', function( e)
            {
                ...
                if ( el[ 0 ] )
                {
                    console.log("mousedown touchstart setting.draggable: " + setting.draggable)
 ...
 * @param {isDrag} possible draggable
 * */
function MenuEditor(idSelector, options, isDrag) {
    var $main = $("#" + idSelector);
    var settings = {
        labelEdit: '<i class="fas fa-edit clickable"></i>',
        labelRemove: '<i class="fas fa-trash-alt clickable"></i>',
        textConfirmDelete: '메뉴 삭제시 하위메뉴와 적용된 관리자의 권한도 같이 삭제 됩니다. 삭제하시겠습니까? ',
    ...
    /* btnRemove click event customize for menu data delete */
    $(document).on('click', '.btnRemove', function (e) {
        e.preventDefault();

        if (confirm(settings.textConfirmDelete)){
            var currentItem = $(this).closest('li').data();
            var menuNo = currentItem.menu_no;
            var upMenuNo = currentItem.up_menu_no;
            ...
            $.ajax({
                url: '/cm/system/menu/deleteMenuItemAction',
                type: 'POST',
                data: param,
                dataType : "json",
                success: function(data){
                    console.log("deleteMenuItemAction result: " + data);
    ...
    $(document).on('click', '.btnEdit', function (e) {
        ...
        console.log(".btnEdit clicked value: " + authType);

        if (authType == "A") {
            $('input:radio[name="auth_type_radio"]').filter('[id="auth_type_all"]').prop('checked', true);
            $('input:radio[name="auth_type_radio"]').filter('[id="auth_type_read"]').prop('checked', false);
            $('input:radio[name="auth_type_radio"]').filter('[id="auth_type_denied"]').prop('checked', false);
        } else if (authType == "R") {
            $('input:radio[name="auth_type_radio"]').filter('[id="auth_type_read"]').prop('checked', true);
            $('input:radio[name="auth_type_radio"]').filter('[id="auth_type_all"]').prop('checked', false);
            $('input:radio[name="auth_type_radio"]').filter('[id="auth_type_denied"]').prop('checked', false);
        } else if (authType == "N") {
            $('input:radio[name="auth_type_radio"]').filter('[id="auth_type_denied"]').prop('checked', true);
            $('input:radio[name="auth_type_radio"]').filter('[id="auth_type_read"]').prop('checked', false);
            $('input:radio[name="auth_type_radio"]').filter('[id="auth_type_all"]').prop('checked', false);
        }

        var menuTargetType = $("#menu_target_type").val();
        console.log(".btnEdit clicked menuTargetType value: " + menuTargetType);

        if (menuTargetType == "ALL") {
            $('input:checkbox[id="menu_target_system"]').prop('checked', true);
            $('input:checkbox[id="menu_target_complex"]').prop('checked', true);
        } else if (menuTargetType == "SYSTEM") {
            $('input:checkbox[id="menu_target_system"]').prop('checked', true);
            $('input:checkbox[id="menu_target_complex"]').prop('checked', false);
        } else if (menuTargetType == "COMPLEX") {
             $('input:checkbox[id="menu_target_system"]').prop('checked', false);
             $('input:checkbox[id="menu_target_complex"]').prop('checked', true);
         } else if (menuTargetType == "NONE") {
             $('input:checkbox[id="menu_target_system"]').prop('checked', false);
             $('input:checkbox[id="menu_target_complex"]').prop('checked', false);
         }
    });
    ...
    function TButtonGroup() {
        ...
        var $btnEdit = TButton({classCss: 'btn btn-bluegreen btn-sm btnEdit', text: settings.labelEdit});
        var $btnRemv = TButton({classCss: 'btn btn-brown btn-sm btnRemove', text: settings.labelRemove});
